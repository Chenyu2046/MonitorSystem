# Windows Docker 基准测试记录（2026-08-12）

## 结论

本次唯一可用于简历或对外说明的有效基线为：在 Windows Docker Linux
运行环境中，模拟 10 个节点以 1 秒周期向 Manager 推送监控数据，连续
30 秒完成 300 次真实 gRPC -> Manager -> MySQL 上报与持久化；请求成功率
为 100%，端到端延迟 P50/P95/P99 分别为 81.615/157.418/201.028 ms。
...
| P50 端到端延迟 | 81,615 us（81.615 ms） |
| P95 端到端延迟 | 157,418 us（157.418 ms） |
| P99 端到端延迟 | 201,028 us（201.028 ms） |
| gRPC 状态码 | 300 个 `OK`（0） |
...
| 3 | 修复并发数据竞争、将 Compose 数据库与建表脚本统一为 `monitor_db` 后，相同施压条件 | 300/300 成功；P50/P95/P99 为 81.615/157.418/201.028 ms | 精确落库 300 行，Manager 保持运行 | 有效基线。 |
...
> 在 Windows Docker Linux 基准环境中模拟 10 节点以 1s 周期上报，连续 30s
> 完成 300 次 gRPC -> MySQL 端到端指标写入，成功率 100%，延迟
...
因此，在这台机器、该 Docker 运行环境和当前“每条上报同步计算并写入 MySQL”
...
> 因此将 65 reports/s 作为当前同步持久化实现的最高已验证稳定档。

## 优化后 Manager -> MySQL 复测
...
> 75 reports/s 作为当前可复测稳定档。

## 扩展容量与 10 分钟稳定性复测（2026-08-13）
...
| 75 节点、1 s 周期、600 s、首轮错峰 | 45,000 / 45,000 / 45,000 | 13.119 / 459.494 / 676.736 ms | 通过：持续 10 分钟仍保持 P99 < 1 s |
...
结论**。它只说明当前全局串行“指标计算 + 多表同步写库”实现不适合直接承接该级别
的集中突发。

本轮可用于简历的 Pico 风格表述：

> 在 **75 节点、1 s 周期、10 分钟**的端到端稳定性压测中，完成 **45,000 次**
> `gRPC -> Manager -> MySQL` 同步持久化，**成功率与精确落库率均为 100%**，
> P50/P95/P99 为 **13.1/459.5/676.7 ms**；Manager 平均 CPU **8.68%**、峰值内存
> **13.3 MiB**。另在 **100/300 节点**整秒突发对照下记录 P99 **1.30/3.88 s**，
> 明确同步写入架构的容量边界。
